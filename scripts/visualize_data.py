...
def plotly_plot_bar(columns): 

    fig = go.Figure(df =[go.bar(x = columns, )])
    fig.update_traces(marker_line_color='cornflowerblue',
                  marker_line_width=1.)

    fig.update_layout(
        title_text=f"Distribution of {columns}s",height = 500, width = 800)
    fig.show()
